...
This note proposes a Codd-style conformance layer for agent memory engines. It does not claim that memory has never been formalized. Cognitive architectures formalize how cognition works; surveys and taxonomies classify systems; governance frameworks ask who owns a memory. This report asks what a machine must do in order to reconstruct a usable past.
...
**Not generational lineage.** Session memory and personal long-term memory are in scope. Memory across decades and rememberers needs extra primitives (inheritance, social reinforcement, contested ownership, retelling). Those hooks are noted in §10 and left unspecified.
...
That miss is the truncation trap. It is stochastic: the same system will look correct on many conversation-QA items and fail on the items that matter. Section 6, rule M2, forbids candidate generation by a single signal as the sole generator. Section 9 gives a test that plants exactly this trace.
...
| **Signal** | A first-class coordinate of recall (see §4.5). |
...
| **Storage strength \(S\)** | Encoding durability. Non-decreasing except on tombstone (Bjork & Bjork, 1992). `forget` never lowers \(S\). |
...
An admissible query-time fusion (one member of a family; not a law of the model) is given in Appendix A. The Critical question in §11 is whether the model should fix that form.
...
| `recall` | Yes | No | Yes: \(D\) of returned traces (M9 side effect) | Fused reconstruction from a cue |
...
**Language.** The external surface may be an API, tools, or an embedded library. It must express these operations. A caller must not be required to compose results from multiple foreign query languages. Syntax is out of scope for this report (see §11).
...
**M2. Guaranteed access.** If a trace is available and passes the cue’s hard gates, no plan may discard it before fused scoring. Cosine top-\(k\) as the sole candidate generator does not satisfy M2, including the variant “top-100 then re-rank.” That variant is the truncation trap in §3.
...
This report sits next to four literatures.

**Cognitive architectures.** ACT-R (Anderson and colleagues) gives a formal activation equation, spreading activation, partial matching, a retrieval threshold, and decay — the closest ancestor of a memory algebra. Soar (Laird, Newell, Rosenbloom) gives working memory, long-term stores, and chunking as consolidation. Both are theories of how cognition works. This report is a specification for what an engine must do. ACT-R’s activation *is* the retrieval plan; this model additionally constrains plans that would truncate before scoring.

**Human memory theory.** Bjork and Bjork’s new theory of disuse (1992) splits storage strength from retrieval strength and predicts larger storage gains when retrieval is hard. Tulving’s episodic/semantic distinction and the Atkinson–Shiffrin family of tiers inform \(\Pi\), not as a claim that the engine is a brain.

**Agent-memory taxonomies and surveys.** CoALA (Sumers, Yao, Narasimhan, and Griffiths, 2023/24) decomposes language-agent memory into working, episodic, semantic, and procedural stores. Hu et al. (2025) survey forms, functions, and dynamics of agent memory. Both organize the field. Neither states normal forms or disqualifying engine rules.

**Governance.** Li (2026) treats memory as ontological ground and asks who may alter it. That question is complementary and out of scope here.

What this report adds is a closed algebra over a defined \(M\), normal forms, and conformance rules that an implementation can pass or fail.
...
“Native engine” is the pattern this report specifies. A reference implementation may still be **P** on M8 (lineage record format) and M12 (cue-level explanation). Running several foreign stores in one process does not change the orchestrated column: in-process planners still fail M0.

The matrix does not rank usefulness. An orchestration layer can score well on conversation-QA and miss M2 on the planted-trace test in §3.
...
Instruction sets, memory APIs, latency, quantization, and header widths are how one engine is fast. The model remains true if a correct engine is slow. Layout belongs in engine documentation, not in this report.
...
| **Not in this report** | Wire protocol, language grammar, benchmark numbers, engine header layout, dream schedule, product comparison |
...
Atkinson, R. C., and R. M. Shiffrin. 1968. Human memory: a proposed system and its control processes.

Bjork, R. A., and E. L. Bjork. 1992. A new theory of disuse and an old theory of stimulus fluctuation. In *From Learning Processes to Cognitive Processes*.

Codd, E. F. 1970. A relational model of data for large shared data banks. *Communications of the ACM* 13(6).

Codd, E. F. 1985. Is your DBMS really relational? / Does your DBMS run by the rules? *Computerworld*.

Hu, Y., S. Liu, Y. Yue, et al. 2025. Memory in the age of AI agents. arXiv:2512.13564.

Laird, J. E., A. Newell, and P. S. Rosenbloom. 1987–. Soar: an architecture for general intelligence.

Li, Z. 2026. Memory as ontology: a constitutional memory architecture for persistent digital citizens. arXiv:2603.04740.

Sumers, T. R., S. Yao, K. Narasimhan, and T. L. Griffiths. 2023/24. Cognitive architectures for language agents. *TMLR*. arXiv:2309.02427.
